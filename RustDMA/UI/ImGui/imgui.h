...
namespace ImGui
{
    ...
    IMGUI_API ImVec2 CustomCalcTextSize(const char* text, const char* text_end = NULL, bool hide_text_after_double_hash = false, float wrap_width = -1.0f, float font_size = 0.f, ImFont* custom_font = nullptr);
    // Color Utilities
...
enum ImGuiBackendFlags_
{
    ...
    ImGuiBackendFlags_SignedDistanceFonts = 1 << 5,
    // Enable/disabled signed distance font rendering (scaling).
    ImGuiBackendFlags_SignedDistanceShapes = 1 << 6,
    // Enable/disabled signed distance rendering of (rounded) rectangles.
    ImGuiBackendFlags_ProvocingVertexFirst = 1 << 7,
    // Needed for DirectX if the signed distance fields operations are enabled.
    ImGuiBackendFlags_DefaultFast = ImGuiBackendFlags_None,
    // Minimal shader, minimal effects
    ImGuiBackendFlags_DefaultDesktop = (1 << 8) - 1,
    // Full desktop backend: SignedDistanceFonts, SignedDistanceShapes
    // [BETA] Viewports
...
enum ImGuiCol_
{
    ...
    // Start color of shadows of normal windows
    ImGuiCol_WindowShadowEnd,
    // End color of shadows of normal windows
    ImGuiCol_ChildBg,
    ...
    // Start color of shadows of frames
    ImGuiCol_FrameShadowEnd,
    // End color of shadows of frames
    ...
    ImGuiCol_FontShadowStart,
    // Start color of shadows of fonts
    ImGuiCol_FontShadowEnd,
    // End color of shadows of fonts
...
struct ImColor
{
    ...
    constexpr ImColor(float r, float g, float b, float a = 1.0f) : Value(r, g, b, a), r(r), g(g), b(b), a(a)
    {
    }
    ...
    constexpr ImColor(int r, int g, int b, int a = 255) : Value((float)r * (1.0f / 255.0f), (float)g * (1.0f / 255.0f), (float)b * (1.0f / 255.0f), (float)a * (1.0f / 255.0f)), r(r), g(g), b(b), a(a)
    {
    }
...
enum ImDrawListFlags_
{
    ...
    ImDrawListFlags_SignedDistanceFonts = 1 << 4,
    // Enable/disabled signed distance operations.
    ImDrawListFlags_SignedDistanceShapes = 1 << 5,
    // Enable/disabled signed distance operations.
    ImDrawListFlags_ProvocingVertexFirst = 1 << 7,
    // Needed for DirectX if the signed distance fields operations are enabled.
};
...
struct ImFont
{
    ...
    IMGUI_API void AddGlyph(const ImFontConfig* src_cfg, ImWchar c, float x0, float y0, float x1, float y1, float u0, float v0, float u1, float v1, float advance_x, float scale = 1.0f);
