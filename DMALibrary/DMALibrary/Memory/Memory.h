...
private:
	...
	*/
	...
	bool ValidPtr(uintptr_t addr) const
	{
		if (addr > 0x2000000 && addr < 0x7FFFFFFFFFFF && (!HeapCheck || addr > ModuleSectionEnd && addr < ModuleEnd) )
			return true;
		return false;
	}
...
public:
	...
	*/
	...
	//Gets all DTBS of the system, even once that are hidden. the custom way (:
	...
	uint64_t FindSignature(const char* signature_start, const char* signature_end, uint64_t range_start, uint64_t range_end, int PID = 0);
	std::vector<uint64_t> FindAllSignature(const char* signature_start, const char* signature_end, uint64_t range_start, uint64_t range_end, int PID = 0);
	/**
	...
	*/
	...
	uint64_t solve(uint64_t addr, uint32_t offset, uint32_t next)
	{
		uint32_t return_data;
		Read(addr + offset, &return_data, sizeof(return_data));
		return (return_data + addr + next);
	}
	...
	*/
	...
	bool AddScatterWriteRequest(VMMDLL_SCATTER_HANDLE handle, uint64_t addr, T value) const
	{
		...
		if (!ret)
		{
			//	LOG(LIT("failed to prepare scatter write at 0x%p\n"), addr);
		}
		return ret;
	}
	...
	void ExecuteReadScatterNoClear(VMMDLL_SCATTER_HANDLE handle, int pid = 0);

	/*the FPGA handle*/
	VMM_HANDLE vHandle;

	void WriteHeapCheck(const bool& value)
	{
		HeapCheck = value;
	}
};
